...
usage() {
  cat <<'EOF'
Usage:
  scripts/archive/overleaf_sync_images.sh [options]
...
delete_flag=false
include_fits=false
commit_message="Sync images from $(basename "$ROOT_DIR") ($(date +%Y-%m-%d\ %H:%M:%S))"
...
if ! git -C "$abs_clone_dir" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  echo "Overleaf clone not found at: $abs_clone_dir" >&2
  echo "Run scripts/archive/overleaf_connect.sh first." >&2
  exit 1
fi
